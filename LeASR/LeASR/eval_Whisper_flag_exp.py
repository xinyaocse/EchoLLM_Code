    ...
    for item in files:
        for key in config_adapter.config.key_list:
            for value in files[item][key]:
                files[item][key][value]['whisper_result'] = text_list[index]
                index += 1
...
def main():
    ...
    def prepare_dataset(batch):
        ...
        sample = batch['audio']
        batch = processor(
            audio=sample["array"], sampling_rate=sample["sampling_rate"], text=batch["text_upper"].lower()
        )
    ...
    def compute_metrics(pred):
        ...
        write_json_result1(config_adapter.config.json_path, pred_str)
        # write_json_result2(config_adapter.config.json_path, pred_str, eval_dataset.eval_dataset.val_key)
