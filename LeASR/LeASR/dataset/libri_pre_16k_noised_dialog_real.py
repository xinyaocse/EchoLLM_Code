...
class LibriNoised8k(datasets.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, files):
        ...
        for id_, item in enumerate(files):
            # text_upper = filter_extra_spaces(lowercase_and_remove_punctuation(item['transcript_flag']))
            text_upper = filter_extra_spaces(lowercase_and_remove_punctuation(replacer.replace(item['transcript_flag'].lower())))


            audio_path = item["noisy_audio_add_flag"]
