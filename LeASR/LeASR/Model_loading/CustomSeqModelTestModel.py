...
from transformers import add_start_docstrings, PreTrainedModel, SpeechEncoderDecoderConfig, PretrainedConfig, AutoModel, \
...
class CustomSpeechEncoderDecoderModelTEST(PreTrainedModel):
    r"""
    ...
    def forward(
        ...
        **kwargs,
        ...
        if labels is not None and decoder_input_ids is None:
            shift_input_ids = shift_tokens_right(
                    labels, self.config.pad_token_id, self.config.decoder_start_token_id
                )
            # context_ids = torch.cat((decoder_input_ids, shift_input_ids), dim=1)
        # else:
        #     # shift_input_ids = (
        #     #     torch.ones([decoder_input_ids.shape[0], 1],
        #     #                dtype=torch.long).to(decoder_input_ids.device).fill_(self.config.decoder_start_token_id)  # plus one for bos
        #     # )
        #     context_ids = decoder_input_ids
        # # decoder_input_ids = decoder_input_ids[:, -40:]
        #
        # # labels = torch.cat((decoder_input_ids[:, -5:], labels[:, 0:56]), dim=1)
        # # Decode
        # if labels is not None:
        #     empty_targets = (
        #         torch.ones([decoder_input_ids.shape[0], decoder_input_ids.shape[1]],
        #                    dtype=torch.long).to(context_ids.device).fill_(-100)  # plus one for bos
        #     )
