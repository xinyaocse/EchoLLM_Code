...
def write_json_result(json_path, text_list, is_aft_pre=True):
    files = get_result(json_path)
    for index, item in enumerate(files):
        if is_aft_pre:
            files[item]['asr_text_context'] = text_list[index]
        else:
            files[item]['asr_text_no_context'] = text_list[index]
...
def main():
    ...
    def prepare_dataset(batch):
        ...
        # Combine features
        combined_input = np.concatenate([inputs1["input_values"][0], inputs2["input_values"][0]])
        
        batch['input_values'] = combined_input
        batch["input_length"] = len(combined_input)
        
        # Process text labels (keep unchanged)
        input_str = batch["text"].lower()
        batch["labels"] = tokenizer(input_str).input_ids
        return batch
    
    raw_datasets = get_eval_datasets(False, True, True)
    ...
    def compute_metrics(pred):
        ...
        print(pred_str)
        write_json_result("./ASR/datasets/asr_test_219/audio_pre.json", pred_str, True)
