...
def japanese_wer(ref: str, hyp: str) -> float:
    """
    Calculates character-based Word Error Rate (simplified version)

    :param ref: Reference text (ground truth)
    :param hyp: Recognized text (hypothesis)
    :return: WER (0.0-1.0)
    """
    # Remove spaces (modify here if punctuation needs to be preserved)
    ref = ref.replace(" ", "")
    hyp = hyp.replace(" ", "")
...
training_args = Seq2SeqTrainingArguments(
    output_dir="/root/public/dev8T/username/dataset_text_audio/pretrained_models/models/openAI_out_JP",
...
class DataCollatorSpeechSeq2SeqWithPadding:
    ...
    def __call__(
        ...
        # First process audio features as PyTorch tensors
        input_features = [
        ...
        # Trim bos token if previously added
...
def main():
    ...
    raw_datasets = load_data_by_path(
        "/root/code_project/speech_asr/dataset/libri_pre_16k_noised_dialog_JP.py",
    ...
    def prepare_dataset(batch):
        ...
        batch = processor(
            ...
            # Remove lowercase conversion for Japanese
    ...
    print("Finished processing Whisper data")
    ...
    def compute_metrics(pred):
        ...
        return {"wer": sum(ortho_wer_samples)/len(ortho_wer_samples)}
